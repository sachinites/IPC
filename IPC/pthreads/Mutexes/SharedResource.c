...
#define THREAD_NUM 4
#define RESOURCE_CAPACITY 2 // adivisable to keep resource capacity less than THREAD_NUM
...
void init_resource(resource_t *resource, 
		   fnptr access_resource_fn_fnptr, 
		   int resource_cap,
		   void *resource_entity)
{
 resource->resource = (void *)resource_entity; // assign pointer to some resource here
...
int 
semaphore_inc(resource_t *resource)
{
...
int 
semaphore_dec(resource_t *resource)
{
  pthread_mutex_lock(&resource->semaphore_mutex);
  resource->semaphore--;
  pthread_mutex_unlock(&resource->semaphore_mutex);
...
/*creating Slave threads threads*/
 ...
 for (i = 1; i < THREAD_NUM; i++) {
 th_arg.thid = i;
 if (pthread_create(&thread[i].pthread_handle, &thread[i].attr, Slave_thread, (void*)&th_arg))
 {
 ...
 case 1:
{
	...
	if(block_thread(gl_resource.block_pool, inp_th_id)==0){
		printf("Master thread added thread %d to gl_blocked_th_pool, at index = %d\n",
			inp_th_id , am_i_in_block_pool_mutex(inp_th_id, *gl_resource.block_pool));
	}
        break;
}
 case 2:
...
while(++i)
{
 if(am_i_in_block_pool_mutex(thid, *(resource->block_pool)) > -1){
	printf("Thread %d realised that is has been black listed, hence blocking itself\n", thid);
